    ...
    <div>
      ...
      <div className='grid grid-cols-5 gap-5 sm:grid-cols-1 md:grid-cols-1 p-5'>
        {data?.results?.map((movie) => {
          return <div key={movie.id} className='w-[200px] md:w-[100%] rounded-md cursor-pointer hover:scale-105 transition-all delay-75 shadow-lg' onClick={() => nav(`/movie/detail/${movie.id}`)}>
            {movie.poster_path != null ?
              <img className='sm:h-[150px] sm:w-[100px] h-[300px] w-[200px] rounded-md' src={`https://image.tmdb.org/t/p/w600_and_h900_bestv2/${movie.poster_path}`} alt="" /> : <img className='h-[300px] w-[200px] rounded-md' src="/assets/images/noimg.jpg" alt="" />
            }
